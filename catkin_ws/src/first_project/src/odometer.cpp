...
void odometerCallback(const geometry_msgs::PointStamped::ConstPtr& msg){
    ...
    double delta_lat = y / 111111.0;  // meters per degree latitude
    double delta_lon = x / (111111.0 * cos(origin_lat * 0.01744444444));  // adjust for longitude lines
    lat = origin_lat + delta_lat;
    lon = origin_lon + delta_lon;
    ROS_INFO("%.20f, %.20f",lat, lon);
